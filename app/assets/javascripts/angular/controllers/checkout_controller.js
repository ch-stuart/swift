...
SwiftApp.controller('CheckoutCtrl', [
  ...
  function saleChargeSuccessCallback(response) {
    ...
    SaleService
      .create({
        ...
        gift_certificate_guid: $scope.giftCertCode,
  ...
  $scope['onCouponCodeBtnClicked'] = function() {
  ...
  $scope['onGiftCertCodeBtnClicked'] = function() {}

  $scope['onGiftCertCodeChanged'] = function() {
    var guid = $scope.giftCertCode;

    if (!guid) {
      $scope.giftCertError = null;
      CartService.nullGiftCertValue();
    } else if (guid.length === 8) {
      CartService
        .getGiftCertValue(guid)
        .then(
          function success(response) {
            $scope.giftCertError = null;

            if (response.remainingAmount === 0) {
              $scope.giftCertError = "No credit is left on this gift certificate.";
            }
          },
          function error() {
            $scope.giftCertError = "Could not find gift certificate.";
          }
        );
    } else {
  ...
  $scope['onGiftCertCodeBlur'] = function() {
    var guid = $scope.giftCertCode;
